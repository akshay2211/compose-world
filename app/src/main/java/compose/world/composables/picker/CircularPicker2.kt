    ...
    Box(
        ...
        LazyColumn(
            ...
            itemsIndexed(items = items) { index, item ->
                val thisScrollOffset = kotlin.runCatching { index * (itemHeightPx + listState.layoutInfo.visibleItemsInfo.find { it.index == index }?.offset!!) / itemHeightPx }.getOrDefault(1F)

//                val distanceFromCenter = //animatedSelectedItemIndex - index +
//                        firstVisibleItemScrollOffset / LocalDensity.current.run { itemHeight.toPx() } // + if (isSelectedItem) 0F else (firstVisibleItemScrollOffset / LocalDensity.current.run { itemHeight.toPx() })
...
//                )
                ...
                Box (
                    ...
                    Text(
                        modifier = Modifier
                            .graphicsLayer {
                                val rotation = (thisScrollOffset * 15F) //.coerceAtMost(45F)
//                                if (index == selectedItemIndex) return@graphicsLayer

                                rotationZ = -rotation
                                translationX = -rotation * 2 * thisScrollOffset
